...
function getErrorMessage(error) {
    return error?.response?.data?.error?.message || 'Unable to load project activity.';
}

export default function ProjectsTimelinePanel({ query }) {
    ...
    return (
        <PanelWrapper
            ...
            description="Monthly count of projects created on the platform."
        >
            {query.isLoading ? (
                <div
                    className="min-h-64 animate-pulse rounded bg-background-tertiary"
                    role="status"
                    aria-label="Loading projects timeline"
                />
            ) : null}
            {query.isError ? (
            ...
            {!query.isLoading && !query.isError ? (
                <div className="relative min-h-64">
                    <Bar
                        aria-label="Project count for the last 15 months"
                        role="img"
                        data={{
